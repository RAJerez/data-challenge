# loaders
# does what was investigated in data_exploratory.ipynb
...
class CineInsightsLoader(BaseLoader):
    ...
    def load_table(self, file_path):
        ...
        df_pantalla_butaca = df.groupby("provincia", as_index=False).sum()[
            ["provincia", "pantallas", "butacas"]
        ]
        df_incaa = (
            df[df["espacio_incaa"] == "Si"]
            .groupby("provincia", as_index=False)
            .count()[["provincia", "espacio_incaa"]]
        )
        insights_df = pd.merge(
            df_pantalla_butaca, df_incaa, on="provincia", how="right"
        ).fillna(0)
